...
O Padrão de segurança de dados do setor de cartões de pagamento (PCI DSS) é um conjunto de regras contratuais que buscam proteger os dados de pagamento do titular do cartão de pagamento durante uma transação e reduzir a fraude. 
Em teoria, o **PCI DSS** é um padrão voluntário. No entanto, na prática, qualquer empresa que armazena, processa ou transmite dados do portador do cartão que não esteja em conformidade com o padrão PCI DSS pode enfrentar taxas de transação significativamente mais altas, multar até US $ 500.000 e, em circunstâncias extremas, perder a capacidade de processar cartões de pagamento.
### Criptografica
As empresas que **importam ou exportam produtos de criptografia comercial** estão sujeitas a regulamentações que são supervisionadas pelo Departamento de Indústria e Segurança do Departamento de Comércio.

As restrições de exportação para estados invasores e organizações terroristas podem estar em vigor devido a preocupações de segurança nacional.  

Além disso, alguns países podem decidir restringir a importação de tecnologias de criptografia devido a preocupações de que:

  * A tecnologia pode conter uma vulnerabilidade da segurança ou de backdoor.
  * Os cidadãos podem usar essa tecnologia para se comunicar de forma anônima e evitar o monitoramento pelas autoridades.
...
Existem várias leis nos EUA que exigem que as empresas notifiquem os indivíduos caso ocorra uma violação de seus dados pessoais.
...
O Guru solicitou que você informe a equipe de RH da @Apollo sobre as principais leis e regulamentações relacionadas à segurança digital que se aplicam ao setor de que precisam estar cientes.
